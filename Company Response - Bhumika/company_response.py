...
def train_and_evaluate(model, dataloader_train, dataloader_val, optimizer, scheduler, epochs=4):
    """
    Trains and evaluates the Roberta model with progress updates.
    """
    ...
    for epoch in range(epochs):
        ...
        print(f"Training Loss: {avg_train_loss:.4f}, Training Accuracy: {train_f1:.4f}")
        ...
        print(
            f"Validation Loss: {avg_val_loss:.4f}, Validation F1-Score: {val_f1:.4f}")
        print("-" * 30)
...
def main():
    ...
    # output_dir = './bert_class_model_final'
    output_dir = './roberta_class_model_final'
    model.save_pretrained(output_dir)
    tokenizer.save_pretrained(output_dir)
    print("Sample Predictions:", output_dir)
