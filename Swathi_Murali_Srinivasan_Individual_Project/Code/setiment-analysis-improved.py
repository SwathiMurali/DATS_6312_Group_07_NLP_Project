...
class SentimentPredictor:
    def __init__(self, model_path='sentiment_model', tokenizer_path='sentiment_tokenizer', max_len=128):
    ...
    def predict_sentiment(self, texts):
        # Prepare dataset in batches to avoid memory issues
        sentiments = []
        batch_size = 32
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # Convert to numpy and map to sentiment labels
            sentiment_map = {0: 'Negative', 1: 'Neutral', 2: 'Positive'}
            batch_sentiments = [sentiment_map[pred.item()] for pred in batch_predictions]
    ...
    def add_sentiment_column(self, df):
        ...
        df['sentiment'] = self.predict_sentiment(df['Processed Narrative'])
        
        # Verify sentiment distribution
        print("Sentiment Distribution:")
        print(df['sentiment'].value_counts(normalize=True))
        
        # Save DataFrame with sentiment column
...
df = predictor.add_sentiment_column(df)
# %%
df.head()
# %%
